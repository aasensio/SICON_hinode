...
import model_x2_improved as model
import time
import shutil
import nvidia_smi
import argparse
...
class dataset_spot(torch.utils.data.Dataset):
    def __init__(self, n_training=10000, n_pixels=32, image_range=[0,10]):
       ...
       # self.stokes += np.random.normal(loc=0, scale=self.noise_scale, size=self.stokes_inv.shape)
        ...
        f = h5py.File(f'{self.root}/rempel_vzminus_model_spat_degraded_sir_hinode_x2.h5', 'r')
        ...
        f = h5py.File(f'{self.root}/rempel_vzminus_model_invert_spat_degraded_sir_hinode_x2.h5', 'r')
        T = np.transpose(f['model'][:,:,1,:], axes=(2,0,1))
        vz = np.transpose(f['model'][:,:,3,:], axes=(2,0,1))
        tau = np.transpose(f['model'][:,:,0,:], axes=(2,0,1))
        logP = np.log10(np.transpose(f['model'][:,:,2,:], axes=(2,0,1)))
        ...
        f = h5py.File(f'{self.root}/cheung_vzminus_model_spat_degraded_sir_hinode_x2.h5', 'r')
        T = np.transpose(f['model'][:,:,1,:], axes=(2,0,1))
        vz = np.transpose(f['model'][:,:,3,:], axes=(2,0,1))
        tau = np.transpose(f['model'][:,:,0,:], axes=(2,0,1))
        logP = np.log10(np.transpose(f['model'][:,:,2,:], axes=(2,0,1)))
        ...
        # Setup
        #--------------------        
        _, self.nx_rempel, self.ny_rempel = self.stokes.shape
        self.n_phys, _, _ = self.phys.shape

        _, self.nx_cheung, self.ny_cheung = self.stokes_cheung.shape
        self.n_phys, _, _ = self.phys_cheung.shape
    ...
    def __getitem__(self, index):
        ...
        if (self.flip_snapshot[index] == 0):
            ...
            target = self.phys[:,2*self.top[index]:2*self.top[index] + 2*self.n_pixels, 2*self.left[index]:2*self.left[index]+2*self.n_pixels]
            ...
            target = self.phys_inv[:,2*self.top[index]:2*self.top[index] + 2*self.n_pixels, 2*self.left[index]:2*self.left[index]+2*self.n_pixels]
                        
            target = (target - self.phys_min[:,None,None]) / (self.phys_max[:,None,None] - self.phys_min[:,None,None])
        else:
            input = self.stokes_cheung_noise[:,self.top[index]:self.top[index] + self.n_pixels, self.left[index]:self.left[index]+self.n_pixels]
                        
            target = self.phys_cheung[:,2*self.top[index]:2*self.top[index] + 2*self.n_pixels, 2*self.left[index]:2*self.left[index]+2*self.n_pixels]
